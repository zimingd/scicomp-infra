...
function provision {
  local provision_cmd=$1
  local message=$($provision_cmd 2>&1 1>/dev/null)
  ...
  if [[ $error_code -ne 0 && $message =~ .*"AlreadyExistsException".* ]]; then
    ...
    if [[ status_code -eq 255 ]]; then
      ...
      local EC2_IP="$(aws cloudformation describe-stacks --stack-name $STACK_NAME | jq -r '.Stacks[0].Outputs[0].OutputValue')"
